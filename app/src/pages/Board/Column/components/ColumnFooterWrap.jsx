...
export function ColumnFooterWrap({ column, index, columnIndex }) {
  ...
  const context = { columnId: column.id, columnIndex: index };

  return (
    <div onDragOver={onDragOver} className={cl(styles.columnWrap, styles.grow, classes)}>
      <FormWrap text="NEW TASK">{({ onClose }) => <TaskForm onClose={onClose} context={context} />}</FormWrap>
    </div>
  );
}
