...
function useDndHeader(columnIndex) {
  const { isDragAndDrop, onDragOverColumn, onDragLeave } = useContext(DragAndDropContext);
  const ref = useRef();
...
export const ColumnHeader = observer(_ColumnHeader);

function _ColumnHeader({ title, color, tasks = [], openForm, onDelete, onTransfer, columnIndex, isLast }) {
  const { ref, onDragLeave, onDragOver } = useDndHeader(columnIndex);

  return (
    <div onDragLeave={onDragLeave} onDragOver={onDragOver} className={styles.columnWrap}>
      <div className={styles.column}>
        <div className={styles.header} style={{ borderTop: `4px solid ${color}` }}>
          <div className={styles.title}>
            <div className={styles.titleWrap}>
              <span className={styles.spanTitle}>{title}</span>
              <div className={styles.count}>{tasks.length}</div>
            </div>
            <div onClick={openForm} style={{ background: color }} className={styles.plus} />
          </div>

          <div className={styles.btnWrap}>
            {columnIndex ? (
              <ChevronButton onClick={onTransfer(-1)} direction="left" leftExtreme />
            ) : (
              <div style={{ width: 25 }} />
            )}
            ...
            {isLast ? <div style={{ width: 25 }} /> : <ChevronButton onClick={onTransfer(1)} direction="right" />}
          </div>
        </div>
      </div>
